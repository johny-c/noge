...
def make_data(dataset, n_graphs, print_every=50):

    if dataset not in SYNTHETIC_DATASETS:
        raise ValueError(f"Unknown dataset dataset: {dataset}. Must be one of {SYNTHETIC_DATASETS}.")
    ...
    if dataset == 'barabasi':
        ...
        raise ValueError(f"Unknown type of graphs: {dataset}")
...
def cfg():
    dataset = 'maze'
    n_graphs = 500 if dataset in ('barabasi', 'maze') else None
    print_every = 50
    data_dir = DATA_DIR
    seed = 123
...
@ex.automain
def main(dataset, n_graphs, print_every, data_dir, _log):

    _log.info(f"Making paths...")
    data_dir = Path(data_dir)
    data_dir.mkdir(exist_ok=True)
    path = data_dir / f"{dataset}.pkl"

    _log.info(f"Generating {dataset} graphs...")
    data = make_data(dataset, n_graphs, print_every=print_every)
